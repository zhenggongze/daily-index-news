    ...
    newsList.forEach(function(news) {
        ...
        for (var ci = 0; ci < industryCats.length; ci++) {
            ...
            if (keywords[cat].some(function(k) { return title.includes(k); })) {
                ...
                if (!isExcluded) { matched = cat; break; }
            }
        }
        if (!matched && keywords.broad.some(function(k) { return title.includes(k); })) {
            ...
            if (!isBroadExcluded) matched = 'broad';
        }
        if (!matched) return;
...
async function main() {
    ...
    for (var cat2 in categories) {
        categories[cat2].forEach(function(item) { usedTitles[item.t] = true; });
    }
    var uncategorized = newsList.filter(function(n) { return !usedTitles[n.title]; });
    uncategorized.slice(0, 20).forEach(function(n, idx) {
        console.log((idx + 1) + '. ' + n.title);
    });
    console.log('\n未分类总数: ' + uncategorized.length);
}
